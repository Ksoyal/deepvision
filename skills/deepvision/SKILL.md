...
输出为锚点文本,每个元素形如 `[id] 标签 bbox=(x0,y0,x1,y1) text="..."`,坐标归一化到 `[0,1]`,原点在左上;基元按阅读顺序(上→下、左→右)排列。关系形如 `a --above--> b`:语义关系(labels/points_to/part_of)来自视觉模型,几何关系(above/left_of/contains 等)由坐标确定性推导。基于这些 id 和坐标回答,不要凭空猜测位置。
