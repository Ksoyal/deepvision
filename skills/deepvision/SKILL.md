...
DeepVision 把图片转成机器可精确引用的层级结构化表示:上层是可读的语义单元(composites),下层是带稳定 id 和归一化坐标 `[0,1]` 的视觉基元(primitives),并包含必要的空间逻辑关系。
...
输出为锚点文本,优先阅读 `# 语义单元`:每个语义单元形如 `[id] 标签 role=... bbox=(x0,y0,x1,y1) text="..." children=[...]`,用于理解题目、表格、表单、段落、流程节点等整体结构。再阅读 `# 视觉基元`:每个基元形如 `[id] 标签 bbox=(x0,y0,x1,y1) parent=... text="..."`,用于精确定位。坐标归一化到 `[0,1]`,原点在左上;基元按阅读顺序(上→下、左→右)排列。关系形如 `a --above--> b`:语义关系(labels/points_to/part_of)来自视觉模型,几何关系(above/left_of/contains 等)由坐标确定性推导,且限制在同一父语义单元内。基于这些 id、text 和坐标回答,不要凭空猜测位置。
